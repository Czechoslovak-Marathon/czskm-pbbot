...
pub async fn get_latest_run(runner: &str) -> Result<Option<Run>> {
    let request_url = format!("https://www.speedrun.com/api/v1/users/{runner}/personal-bests", runner = runner);
    ...
    match data.data {
        Some(runs) => {
            let latest_run = runs.into_iter().fold(None, |max, x| match max {
                None => Some(x),
                Some(y) => Some(if y.run.status.verify_date.is_none() || x.run.status.verify_date > y.run.status.verify_date { x } else { y }),
            });
            Ok(latest_run)
        }
        None => Ok(None)
...
pub async fn get_category_data(category: &str) -> Result<Option<Category>> {
    let request_url = format!("https://www.speedrun.com/api/v1/categories/{category}", category = category);
    let response = reqwest::get(&request_url)
        .await?
        .text()
        .await?;
...
pub async fn get_level_data(level: &str) -> Result<Option<Level>> {
    let request_url = format!("https://www.speedrun.com/api/v1/levels/{level}", level = level);
    let response = reqwest::get(&request_url)
        .await?
        .text()
        .await?;
...
pub async fn get_variables(values: HashMap<String, String>) -> Result<Option<String>> {
    ...
    for (key, value) in values {
        let request_url = format!("https://www.speedrun.com/api/v1/variables/{variable}", variable = key);
        let response = reqwest::get(&request_url)
            .await?
            .text()
            .await?;
